...
export default function CountryModal({closeModal , onSelect }) {
                ...
                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div className="p-4 md:p-5 space-y-4">
                        <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                            Please select a country from the list below. The available options cover countries from
                        ...
                        <div>
                            <select id="country-select" name="country" style={{border: 'none'}} value={countrycode} onChange={handleChange}
                                ...
                                {[
                                    ...
                                    <option key={code} value={code.toUpperCase()}>{code.toUpperCase()}</option>
                                ))}
                            </select>
                        </div>
                    </div>
                    <div className="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                        <button data-modal-hide="default-modal" type="button" onClick={countrycode !== "" ? handleSubmit : undefined}
                                className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            {countrycode === "" ? "Select" : "Thrive"}
                        </button>
                    </div>
                </div>
            </div>
